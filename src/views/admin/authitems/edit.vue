<template>
  <div>
    <div class="vx-col w-full mb-base vs-con-loading__container" id="userDetails">
      <vx-card>


        <ValidationObserver ref="form" v-slot="errors">
          {{setAllErrors(errors.errors)}}


          <div class="vx-row mb-2">

            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="name"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  icon="icon-user"
                  icon-no-border
                  val-icon-success="done"
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.name')"
                  v-model="authitem.name"
                  :success=" !v.errors[0] ? true : false"
                  :danger=" v.errors[0] ? true : false"/>
                <span class="text-danger text-sm">{{ v.errors[0] }}</span>
              </ValidationProvider>
            </div>
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="description_ru"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  icon="icon-user"
                  icon-no-border
                  val-icon-success="done"
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.description_ru')"
                  v-model="authitem.description_ru"
                  :success=" !v.errors[0] ? true : false"
                  :danger=" v.errors[0] ? true : false"/>
                <span class="text-danger text-sm">{{ v.errors[0] }}</span>
              </ValidationProvider>
            </div>

          </div>
          <div class="vx-row mb-2">

            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="description_uz"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  icon="icon-user"
                  icon-no-border
                  val-icon-success="done"
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.description_uz')"
                  v-model="authitem.description_uz"
                  :success=" !v.errors[0] ? true : false"
                  :danger=" v.errors[0] ? true : false"/>
                <span class="text-danger text-sm">{{ v.errors[0] }}</span>
              </ValidationProvider>
            </div>

          </div>

        </ValidationObserver>
      </vx-card>

    </div>
    <div class="vx-col w-full mb-base">
      <vx-card>
        <vs-button class="mr-3 mb-2 float-right" @click="save()">{{$t('actions.update')}}</vs-button>
        <vs-button color="warning" type="border" class="mb-2">{{$t('actions.back')}}
        </vs-button>
      </vx-card>
    </div>

  </div>
</template>
<script>
  import CrudService from "../../../services/crud.service";
  import RoleService from "../../../services/role.service";

  export default {
    name: "edit",
    data() {
      return {
        authitem: {
          description_ru: "",
          description_uz: "",
          name: ""
        },
        roleList: [],
        allErrors: null,
      }
    },
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.AuthItem = this.authitem;
        await CrudService.update('auth-item', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/admin/auth-items').catch(() => {
              })
            }, 1000);
          } else {
            vm.makeToast(vm.$t('actions.messages.all_field_required'), vm.$t('actions.error'), 'danger');
          }

        });
      },
      setAllErrors(errors) {
        this.allErrors = errors;
      },
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('auth-item', this.$route.params.id).then(response => {
            this.authitem = response.data;
          });
        }

      },

    },
    async mounted() {
      await this.getAuthItem();
    },
    beforeCreate() {
      if (this.$route.params.id) {
        this.$route.meta.breadcrumb[this.$route.meta.breadcrumb.length - 1].title = this.$route.params.id;
      }
    },
    computed: {
      formValid() {
        let valid = false;
        for (const [key, value] of Object.entries(this.allErrors)) {
          if (value.length === 0) {
            valid = true;
          } else {
            valid = false;
          }
        }
        return valid;
      }
    }
  }
</script>
<style scoped>
  .con-select {
    width: 100% !important;
  }
</style>
