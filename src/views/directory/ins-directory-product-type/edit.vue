            ...
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="name_uz"
                ...
                <vs-input
                  ...
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.namu_uz')"
                  v-model="item.namu_uz"
                  :success=" !v.errors[0] ? true : false"
                  :danger=" v.errors[0] ? true : false"/>
                <span class="text-danger text-sm">{{ v.errors[0] }}</span>
              </ValidationProvider>
  ...
  export default {
    ...
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.InsDirectoryProductType = this.item;
        await CrudService.update('ins-directory-product-type', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/directory/ins-directory-product-type').catch(() => {
              })
            }, 1000);
          } else {
      ...
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('ins-directory-product-type', this.$route.params.id).then(response => {
