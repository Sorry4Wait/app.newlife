      ...
      <vs-table ref="table"  v-model="selected" :max-items="filterData.limit" search :data="users" :noDataText="$t('actions.messages.empty_table')">
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td :data="data[indextr].namu_uz">
              {{ data[indextr].namu_uz}}
            </vs-td>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteEl() {
        ...
        await CrudService.delete('ins-directory-product-type', this.currentElement.id).then(response => {
          if (response.data === 1) {
      ...
      edit(el) {
        this.$router.push('/directory/ins-directory-product-type/update/' + el.id)
      },
      create() {
        this.$router.push('/directory/ins-directory-product-type/create')
      },
      async getList() {
        let vm = this;
        vm.setLoadingDiv('data-list-list-view');
        await CrudService.index('ins-directory-product-type', this.filterData).then(response => {
