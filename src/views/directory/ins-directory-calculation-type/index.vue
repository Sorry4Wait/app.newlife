    ...
    <div id="data-list-list-view" class="data-list-container vs-con-loading__container">
      ...
      <vs-table ref="table"  v-model="selected" :max-items="filterData.limit" search :data="users" :noDataText="$t('actions.messages.empty_table')">
        ...
        <template slot="thead">
          ...
          <vs-th sort-key="id_product_type">{{$t('table.id_product_type')}}</vs-th>
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td :data="data[indextr].id_product_type">
              {{ data[indextr].id_product_type }}
            </vs-td>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteEl() {
        ...
        await CrudService.delete('ins-directory-calculation-type', this.currentElement.id).then(response => {
          if (response.data === 1) {
      ...
      edit(el) {
        this.$router.push('/directory/ins-directory-calculation-type/update/' + el.id)
      },
      create() {
        this.$router.push('/directory/ins-directory-calculation-type/create')
      },
      async getList() {
        let vm = this;
        vm.setLoadingDiv('data-list-list-view');
        await CrudService.index('ins-directory-calculation-type', this.filterData).then(response => {
