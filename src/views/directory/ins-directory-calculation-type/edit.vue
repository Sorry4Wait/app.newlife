        ...
        <ValidationObserver ref="form" v-slot="errors">
          ...
          <div class="vx-row mb-2">
            ...
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="name_ru"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  ...
                  :label-placeholder="$t('table.name')"
            ...
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="name_uz"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  ...
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.description_ru')"
          ...
          <div class="vx-row mb-2">
            ...
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="id_product_type"
                                  :rules="{'required':true}">
                <vs-input
                  class="w-full"
                  icon-pack="feather"
                  ...
                  val-icon-danger="close"
                  val-icon-warning="warning"
                  :label-placeholder="$t('table.description_uz')"
                  v-model="item.id_product_type"
                  :success=" !v.errors[0] ? true : false"
                  :danger=" v.errors[0] ? true : false"/>
                <span class="text-danger text-sm">{{ v.errors[0] }}</span>
              </ValidationProvider>
  ...
  export default {
    ...
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.InsDirectoryCalculationType = this.item;
        await CrudService.update('ins-directory-calculation-type', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/directory/ins-directory-calculation-type').catch(() => {
              })
            }, 1000);
          } else {
      ...
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('ins-directory-calculation-type', this.$route.params.id).then(response => {
