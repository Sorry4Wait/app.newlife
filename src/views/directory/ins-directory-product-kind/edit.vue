  ...
  export default {
    ...
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.InsDirectoryPrefix = this.item;
        await CrudService.update('ins-directory-product-kind', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/directory/ins-directory-product-kind').catch(() => {
              })
            }, 1000);
          } else {
      ...
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('ins-directory-product-kind', this.$route.params.id).then(response => {
