            ...
            <div class="vx-col w-1/2">
              <ValidationProvider v-slot="v" name="name_ru"
                ...
                <vs-input
                  ...
                  :label-placeholder="$t('table.prefix')"
                  v-model="item.prefix"
  ...
  export default {
    ...
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.InsDirectoryProductKind = this.item;
        await CrudService.update('ins-directory-prefix', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/directory/ins-directory-prefix').catch(() => {
              })
            }, 1000);
          } else {
      ...
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('ins-directory-prefix', this.$route.params.id).then(response => {
