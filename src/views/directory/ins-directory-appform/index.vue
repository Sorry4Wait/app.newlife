  ...
  <div>
    <div id="data-list-list-view" class="data-list-container vs-con-loading__container">
      ...
      <vs-table ref="table"  v-model="selected" :max-items="filterData.limit" search :data="users" :noDataText="$t('actions.messages.empty_table')">
        ...
        <template slot="thead">
          <vs-th sort-key="name_uz">{{$t('table.name_ru')}}</vs-th>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteEl() {
        ...
        await CrudService.delete('ins-directory-appform', this.currentElement.id_department).then(response => {
          if (response.data === 1) {
      ...
      edit(el) {
        this.$router.push('/directory/ins-directory-appform/update/' + el.id_department)
      },
      create() {
        this.$router.push('/directory/ins-directory-appform/create')
      },
      async getList() {
        let vm = this;
        vm.setLoadingDiv('data-list-list-view');
        await CrudService.index('ins-directory-appform', this.filterData).then(response => {
