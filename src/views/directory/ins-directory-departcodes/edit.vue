  ...
  export default {
    ...
    methods: {
      async save() {
        let vm = this;
        let obj = {}
        obj.InsDirectoryDepartcodes = this.item;
        await CrudService.update('ins-directory-departcodes', obj).then(response => {
          if (response.data === true) {
            vm.makeToast(vm.$t('actions.messages.update_success'), vm.$t('actions.success'), 'success');
            setTimeout(() => {
              this.$router.push('/directory/ins-directory-departcodes').catch(() => {
              })
            }, 1000);
          } else {
      ...
      async getAuthItem() {
        if (this.$route.params.id) {
          await CrudService.get('ins-directory-departcodes', this.$route.params.id).then(response => {
