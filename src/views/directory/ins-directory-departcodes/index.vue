  ...
  <div>
    <div id="data-list-list-view" class="data-list-container vs-con-loading__container">
      ...
      <vs-table ref="table"  v-model="selected" :max-items="filterData.limit" search :data="users" :noDataText="$t('actions.messages.empty_table')">
        ...
        <template slot="thead">
          <vs-th sort-key="name_uz">{{$t('table.id_department')}}</vs-th>
          <vs-th sort-key="name_uz">{{$t('table.code')}}</vs-th>
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td :data="data[indextr].id_department">
              {{ data[indextr].id_department }}
            </vs-td>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteEl() {
        ...
        await CrudService.delete('ins-directory-departcodes', this.currentElement.id_department).then(response => {
          if (response.data === 1) {
      ...
      edit(el) {
        this.$router.push('/directory/ins-directory-departcodes/update/' + el.id_department)
      },
      create() {
        this.$router.push('/directory/ins-directory-departcodes/create')
      },
      async getList() {
        let vm = this;
        vm.setLoadingDiv('data-list-list-view');
        await CrudService.index('ins-directory-departcodes', this.filterData).then(response => {
